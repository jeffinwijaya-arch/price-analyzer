...
FIXED_DIAL = {
    ...
    "52506":      "Ice Blue",
    # Datejust 36 WG Jubilee gem-set Mint Green (126234G) — single catalogued dial
    "126234G":    "Mint Green",
}
...
_PREMIUM_REF_MAP = {
    "Tiffany Blue": [
        ...
        # Daytona YG leather Tiffany (explicit — also confirmed via dial_options fallback)
        "126518LN",
    ...
    "Grossular": [
        ...
        # Daytona leather strap variants — Grossular confirmed in dial_options
        "116505", "116515",
    ],
...
_PREMIUM_PATTERNS = [
    ...
    # Meteorite — informal cosmic/space descriptors used by private sellers
    (re.compile(r"\bcosmic\s+(?:dial|face)\b|\bgalactic\s+dial\b", re.I), "Meteorite", 75),
    # Wimbledon
    ...
    # "tiffy" — colloquial shorthand used in HK/Asia social media and quick dealer messages
    (re.compile(r"\btiffy\b",                      re.I),      "Tiffany Blue",     85),
    # Tiffany Blue with authenticity-qualifier prefixes (common in premium secondary-market listings)
    (re.compile(r"\bgenuine\s+tiff(?:any)?\b",     re.I),      "Tiffany Blue",    100),
    (re.compile(r"\bauthentic\s+tiff(?:any)?\b",   re.I),      "Tiffany Blue",     95),
    (re.compile(r"\borig(?:inal)?\s+tiff(?:any)?\b", re.I),    "Tiffany Blue",     95),
    (re.compile(r"\btiffany\s+official\b",         re.I),      "Tiffany Blue",    100),
...
_COLOR_PATTERNS = [
    ...
    # Intense White — specific dial for Datejust 31/special refs; must precede plain White
    (re.compile(r"\bintense\s+white\b|\bint\s+white\b",            re.I), "Intense White"),
...
# Compound triggers ("dial colour:", "dial color:") are listed first so they win
# priority over the bare "dial" alternative — preventing "color" or "colour"
# from being swallowed into the capture group when "dial" alone is matched first.
_DIAL_CONTEXT_RE = re.compile(
    r"(?:dial\s+colou?r|dial\s+color|dial\s+face|colou?r|color|face|cadran|dial)\s*[:\-]?\s*"
    r"([a-z][a-z0-9 '\-]{1,40}?)(?:\s*[,;/|]|$)",
    re.I,
)
